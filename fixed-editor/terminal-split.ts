...
export class TerminalSplitCompositor {
	...
	install(): void {
		...
		this.assertTerminalWritable();
		this.enterTerminalMode();
		this.installEmergencyCleanup();
		this.patchTerminalRows();
		this.patchTuiRenderHooks();
		this.patchInputListener();
		this.patchTerminalWrite();
		this.patchCompositeLineAt();
		this.installed = true;
	}

	private assertTerminalWritable(): void {
	...
	private installEmergencyCleanup(): void {
		this.emergencyCleanup = () => {
			if (!this.disposed) this.restoreTerminalStateForExit();
		};
		process.once("exit", this.emergencyCleanup);
	}

	private patchTerminalRows(): void {
	...
	private patchTuiRenderHooks(): void {
		if (this.originalRender) {
			this.tui.render = (width: number) => this.renderScrollableRoot(width);
		}
		if (this.originalDoRender) {
			this.tui.doRender = () => this.renderWithFixedCluster();
		}
	}

	private renderWithFixedCluster(): void {
	...
	private patchInputListener(): void {
	...
	private patchTerminalWrite(): void {
		this.terminal.write = (data: string) => this.write(data);
	}

	private patchCompositeLineAt(): void {
		if (typeof this.tui.compositeLineAt !== "function") return;
	...
	private getSelectedText(): string {
		...
		const lines = area === "root" ? this.rootLines : this.visibleClusterLines;
